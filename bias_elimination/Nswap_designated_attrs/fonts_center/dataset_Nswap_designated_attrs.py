...
class ilab_Nswap_imgfolder(Dataset):
    ...
    def findN(self, index):
        ...
        # U has different content
        '''different content'''
        U_letter = C_letter if not self.disentangle_content else random.choice(temp_Letters)
        U_size = C_size if not self.disentangle_size else random.choice(temp_Size)
        U_font_color = C_font_color if not self.disentangle_font_color else random.choice(temp_font_color)
        resume_colors = temp_back_colors.copy()
        if U_font_color in resume_colors:
            resume_colors.remove(U_font_color)
        U_back_color = C_back_color if not self.disentangle_back_color else random.choice(resume_colors)
        U_font = C_font if not self.disentangle_style else random.choice(temp_font)
        U_img_name = U_letter + '_' + U_size + '_' + U_font_color + '_' + U_back_color + '_' + U_font + ".png"
        U_img_path = os.path.join(self.root, U_letter, U_size, U_font_color, U_back_color, U_font, U_img_name)
        ...
        return U_img_path, A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path
    def __getitem__(self, index):
        '''there is a big while loop for choose category and training'''
        U_img_path, A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path = self.findN(index)


        U_img = Image.open(U_img_path).convert('RGB')
        A_img = Image.open(A_img_path).convert('RGB')
        B_img = Image.open(B_img_path).convert('RGB')
        C_img = Image.open(C_img_path).convert('RGB')
        D_img = Image.open(D_img_path).convert('RGB')
        ...
        return {'U': U, 'A': A, 'B': B, 'C': C, 'D': D, 'E': E, 'F': F}
...
def return_data(args):
    ...
    if name.lower() == '3dchairs':
        ...
        # dataset = ilab_sup_imgfolder(root, transform)
        # dataset = ilab_threeswap_imgfolder(root, transform)
        dataset = ilab_Nswap_imgfolder(root, args, transform)
