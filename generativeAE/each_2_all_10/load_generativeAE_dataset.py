...
class ilab_Nswap_imgfolder(Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        '''refer'''
        # color 10
        self.fg_dict = {'cyan': (0, 255, 255), 'yellow': (255, 255, 0), 'magenta': (255, 0, 255)}
        self.font_colors = list(self.fg_dict.keys())
        self.fg_label = {}
        for idx, key in enumerate(self.fg_dict):
            self.fg_label[key] = idx

        self.bg_dict = {'red': (255, 0, 0), 'green': (0, 255, 0), 'blue': (0, 0, 255)}
        self.back_colors = list(self.bg_dict.keys())
        self.bg_label = {}
        for idx, key in enumerate(self.bg_dict):
            self.bg_label[key] = idx

        # size 3
        self.Sizes = {'small': 80, 'medium': 100, 'large': 120}
        self.Sizes = list(self.Sizes.keys())
        self.size_label = {}
        for idx, key in enumerate(self.Sizes):
            self.size_label[key] = idx

        # style nearly over 100
        style_list = []
        dir = os.path.join(self.root, 'A', 'medium', 'yellow', 'red')
        if os.path.exists(dir):
            style_list.extend(os.listdir(dir))
        self.All_fonts = list(set(style_list))
        if len(self.All_fonts) == 0:
            raise Exception("Sorry, no styles found")
        self.style_label = {}
        for idx, key in enumerate(self.All_fonts):
            self.style_label[key] = idx

        # letter 52
        self.Letters = [chr(x) for x in list(range(65, 91)) + list(range(97, 123))]
        self.letter_label = {}
        for idx, key in enumerate(self.Letters):
            self.letter_label[key] = idx

    def findN(self, index):
        # random choose a C image
        C_letter  = self.Letters[index]
        C_size = random.choice(self.Sizes)
        C_font_color = random.choice(self.font_colors)
        ...
        temp_font_colors = self.font_colors.copy()# avoid same font_color with C
        temp_font_colors.remove(C_font_color)
        ...
        A_font_color = random.choice(temp_font_colors)
        ...
        '''SAME size'''
        B_size = C_size
        B_font_color = random.choice(temp_font_colors)
        ...
        E_font_color = random.choice(temp_font_colors)
        '''SAME back_color'''
        ...
        F_font_color = random.choice(temp_font_colors)
    ...
    def __getitem__(self, index):
        ...
        for idx, name in enumerate(name_list):
            ...
            labels['bg'].append(self.bg_label[name.split('_')[3]])
            labels['style'].append(self.style_label[name.split('_')[4].split('.')[0]])
...
def return_data(args):
    ...
    if name.lower() == '3dchairs':
        ...
        if args.use_server == True:
            ...
            # root = '/home3/data/fonts_dataset_center'
