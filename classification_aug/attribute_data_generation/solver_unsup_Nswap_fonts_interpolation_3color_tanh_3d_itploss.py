...
class Solver(object):
    ...
    def restore_model(self, resume_iters):
        ...
        Auto_path = os.path.join(self.model_save_dir, self.viz_name, '{}-Auto_little.ckpt'.format(resume_iters))
        ...
        while not out:
            for sup_package in self.data_loader:
                ...
                for _ in range(10):
                    ...
                    for m in range(z_new.size()[1]):
                        for n in range(z_new.size()[0]):
                    ...
                    interpolation_z_2 = interpolation_z[:,
                                        self.z_size_start_dim: self.z_font_color_start_dim]  # 200-400
                    interpolation_z_3 = interpolation_z[:,
                                        self.z_font_color_start_dim: self.z_back_color_start_dim]  # 400-600
                    interpolation_z_5 = interpolation_z[:, self.z_style_start_dim:]  # 800-1000

                    C_z_without4 = torch.cat((C_z_1, C_z_2, C_z_3, C_z_5), dim=1)
                    interpolation_z_without4 = torch.cat(
                        (interpolation_z_1, interpolation_z_2, interpolation_z_3, interpolation_z_5), dim=1)
                ...
                # whole loss
                vae_unsup_loss = recon_loss + self.lambda_combine * combine_sup_loss + self.lambda_unsup * combine_unsup_loss + 0.001*interpolation_loss
                ...
                f.writelines(['\n', '[{}] recon_loss:{:.3f}  combine_sup_loss:{:.3f}  combine_unsup_loss:{:.3f} interpolation_loss:{:.3f}'.format(
                        self.global_iter, recon_loss.data, combine_sup_loss.data, combine_unsup_loss.data, interpolation_loss.data)])
                f.close()
                print('\n',self.global_iter, 'recon:',recon_loss.data, 'combinesup',combine_sup_loss.data, 'combine unsup',combine_unsup_loss.data, 'interpolation_loss:', interpolation_loss.data)
