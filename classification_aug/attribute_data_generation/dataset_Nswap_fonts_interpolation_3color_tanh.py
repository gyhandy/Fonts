...
class ilab_Nswap_imgfolder(Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        # self.paths = make_dataset(self.root)
        # number of letters
        self.C_size = 10 # too much we fix it as the number of letters
        '''refer'''
        # color 3
        self.back_Colors = {'red': (255, 0, 0), 'blue': (0, 0, 255),'green':(0,255,0)}
        self.back_Colors = list(self.back_Colors.keys())
        self.fg_Colors = {'blue1': (0, 0, 255), 'Yellow1': (255, 255, 0),'red1':(255,0,0)}
        self.fg_Colors = list(self.fg_Colors.keys())
        # size 2
        self.Sizes = {'small': 80, 'large': 120}
        self.Sizes = list(self.Sizes.keys())
        # style nearly over 100
        #for dirs in os.walk(os.path.join(self.root, 'a', 'small', 'blue1', 'blue')):
         #   cates = dirs
          #  break
        self.All_fonts = ['ubuntu','waree','freemono']
        print(len(self.All_fonts))
        print(self.All_fonts, len(self.All_fonts))
        # letter 10
        self.Letters = [x for x in ['a','A','b','B','c','C','d','D','e','E']]
    ...
    def findN(self, index):
        ...
        temp_Letters = self.Letters.copy()# avoid same letter with C
        temp_Letters.remove(C_letter)
        temp_Size = self.Sizes.copy()# avoid same size with C
        temp_Size.remove(C_size)
        temp_font_color = self.fg_Colors.copy()# avoid same front_color with C
        temp_font_color.remove(C_font_color)
        temp_back_colors = self.back_Colors.copy()  # avoid same background_color with C and avoid same color with font
        ...
        return A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path,C_back_color, C_size, C_font,C_letter,C_font_color
    def __getitem__(self, index):
        '''there is a big while loop for choose category and training'''
        A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path, C_bg, C_size, C_style,C_letter,C_front= self.findN(index)
        ...
        return {'A': A, 'B': B, 'C': C, 'D': D, 'E': E, 'F': F, 'C_bg': C_bg, 'C_size': C_size, 'C_style': C_style,'C_letter':C_letter,'C_front':C_front}
...
def return_data(args):
    ...
    elif name.lower() == 'font_dataset_small_540':
        if args.use_server == True:
            root = '/lab/tmpig23b/u/he-data/font_dataset_small_540'
        else:
    ...
    elif name.lower() == 'font_dataset_little_240':
        if args.use_server == True:
            root = '/lab/tmpig23b/u/he-data/font_dataset_little_240'
        else:
            # root = '/home2/fonts_dataset_version2rgb'
            root = '/lab/tmpig23b/u/he-data/font_dataset_little_240'
            # root = '/home2/fonts_dataset_half'
