...
class Solver(object):
    def __init__(self, args):
        ...
        # log
        self.log_dir = args.model_save_dir +"/"+ args.viz_name
    ...
    def restore_model(self, resume_iters):
        ...
        Auto_path = os.path.join('/lab/tmpig23b/u/zhix/GSL/checkpoints', self.viz_name, '{}-Auto.ckpt'.format(resume_iters))
        ...
        while not out:
            for sup_package in self.data_loader:
                ...
                for _ in range(10):
                    ...
                    for m in range(20):
                        for n in range(4):
                            z_new[n][m] = random.uniform(A_z_4[n][m],C_z_4[n][m])
                    interpolation_img_z = torch.cat((C_z_1, C_z_2, C_z_3, z_new, C_z_5), dim=1)
                    ...
                    interpolation_list.append(interpolation_img)

                    _, interpolation_z = self.Autoencoder(interpolation_img)
                    interpolation_z_1 = interpolation_z[:, 0:self.z_size_start_dim] # 0-200
                    interpolation_z_2 = interpolation_z[:, self.z_size_start_dim : self.z_font_color_start_dim] # 200-400
                    interpolation_z_3 = interpolation_z[:, self.z_font_color_start_dim : self.z_back_color_start_dim] #400-600
                    interpolation_z_5 = interpolation_z[:, self.z_style_start_dim :] #800-1000

                    C_z_without4 = torch.cat((C_z_1, C_z_2, C_z_3, C_z_5), dim=1)
                    interpolation_z_without4 = torch.cat((interpolation_z_1, interpolation_z_2, interpolation_z_3, interpolation_z_5), dim=1)
                    interpolation_loss += self.MSE_loss(C_z_without4, interpolation_z_without4)
                '''
                ...
                # whole loss
                interpolation_loss *= 1e-4
                vae_unsup_loss = recon_loss + self.lambda_combine * combine_sup_loss + self.lambda_unsup * combine_unsup_loss + interpolation_loss
                ...
                f.writelines(['\n', '[{}] recon_loss:{:.3f}  combine_sup_loss:{:.3f}  combine_interpolation_loss:{:.3f}'.format(
                        self.global_iter, recon_loss.data, combine_sup_loss.data, interpolation_loss.data)])
                f.close()
                ...
                if self.global_iter%self.display_step == 0:
                    pbar.write('[{}] recon_loss:{:.3f}  combine_sup_loss:{:.3f}  combine_interpoaltion_loss:{:.3f}'.format(
                        self.global_iter, recon_loss.data, combine_sup_loss.data, interpolation_loss.data))
                    ...
                    if self.viz_on:
                        ...
                        for m in range(10):
                            self.gather.insert(interpolation=F.sigmoid(interpolation_list[m]).data)
                        self.viz_interpolation()
                        # self.viz_combine(x)
                        self.gather.flush()
    ...
    def save_sample_img(self, tensor, mode):
        ...
        if mode == 'recon':
            ...
            interpolation1.save(os.path.join(dir, '{}-interpolation1.png'.format(self.global_iter)))
            interpolation2.save(os.path.join(dir, '{}-interpolation2.png'.format(self.global_iter)))
            interpolation3.save(os.path.join(dir, '{}-interpolation3.png'.format(self.global_iter)))
            interpolation4.save(os.path.join(dir, '{}-interpolation4.png'.format(self.global_iter)))
            interpolation5.save(os.path.join(dir, '{}-interpolation5.png'.format(self.global_iter)))
            interpolation6.save(os.path.join(dir, '{}-interpolation6.png'.format(self.global_iter)))
            interpolation7.save(os.path.join(dir, '{}-interpolation7.png'.format(self.global_iter)))
            interpolation8.save(os.path.join(dir, '{}-interpolation8.png'.format(self.global_iter)))
            interpolation9.save(os.path.join(dir, '{}-interpolation9.png'.format(self.global_iter)))
            interpolation10.save(os.path.join(dir, '{}-interpolation10.png'.format(self.global_iter)))
