...
from model_share import Generator_fc, Classifier_Latent
from dataset_Nswap_fonts import return_data, val_data_loader, train_data_loader
        ...
        while not out:
            for sup_package in self.data_loader:
                ...
                if self.global_iter % 10000 == 0:
                    ...
                    for (img,label) in self.val_data_loader:
                        index = label.item()
                        img = img.cuda()
                        label = label.cuda()
                        _, img_z = self.Autoencoder(img)
                        letter_z = img_z[:, 0:self.z_size_start_dim]   
                        result =  self.letter_classifier(letter_z)             
                    ...
                    print('Accuracy: {}'.format(correct[0] / total[0]))
                    log = open(self.log_dir + '/acc.txt', 'a')
                    log.write("test:global_iter"+str(self.global_iter)+":\n")
                    log.write(str(acc))
                    log.write('\n')
                    log.close()
                if self.global_iter % 10000 == 0:
                    ...
                    print('Accuracy: {}'.format(correct[0] / total[0]))
                    log = open(self.log_dir + '/acc.txt', 'a')
                    log.write("train:global_iter"+str(self.global_iter)+":\n")
                    log.write(str(acc))
                    log.write('\n')
                    log.close()
                #　save the log
