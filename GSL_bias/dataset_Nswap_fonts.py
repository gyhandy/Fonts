...
class ilab_Nswap_imgfolder(Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        '''refer'''
        # color 10
        self.font_color_set = list({'blue1','green1','pink1','red1','silver1','Yellow1'})
        self.back_color_set = list({'blue','green','pink','red','silver','Yellow'})

        self.All_fonts = list({'ani','dejavusans','freemono','freeserif','karumbi','liberationmono','liberationserif','loma','norasi','notomono','notosanscjkhk',
            'notoserifcjkjp','padauk','padmaabold11','pagul','phetsarathot','purisa','sahadeva','sawasdee','tlwgtypist','ubuntu','waree'})
        # size 3
        ...
        for i in range(len(self.Letters)):
            self.letter_dict[self.Letters[i]]=i
        
        self.size_dict = {'small': 0, 'medium': 1, 'large': 2}
        self.fg_dict = {'blue1':0,'green1':1,'pink1':2,'red1':3,'silver1':4,'Yellow1':5}
        self.bg_dict1 = {'blue':0,'green':1,'pink':2,'red':3,'silver':4,'Yellow':5}
        self.style_dict = {}
        for i in range(len(self.All_fonts)):
            self.style_dict[self.All_fonts[i]]=i

    def findN(self, index):
        # random choose a C image
        with open("/lab/tmpig23b/u/zhix/font_dataset_D/log.txt",'r', encoding='UTF-8') as f:
            Letters_Color_dict = json.load(f)

        C_letter  = self.Letters[index]
        while len(Letters_Color_dict[C_letter]) == 1:
            C_letter = random.choice(self.Letters)

        if len(Letters_Color_dict[C_letter])==1:
            C_G = 'G1'
        elif len(Letters_Color_dict[C_letter])==3:
            C_G = 'G2'
        else:
            C_G = 'G3'
        C_size = random.choice(self.Sizes)
        C_font_color = random.choice(self.font_color_set)
        C_back_color = random.choice(Letters_Color_dict[C_letter])
        C_font = random.choice(self.All_fonts)
        C_img_name = C_G + '_' + C_letter + '_' + C_back_color + '_' + C_size + '_' + C_font_color  + '_' + C_font + ".png"
        C_img_path = os.path.join(self.root, C_G, C_letter, C_back_color, C_size, C_font_color, C_font, C_img_name)
        ...
        temp_font_color = self.font_color_set.copy()# avoid same font_color with C
        temp_font_color.remove(C_font_color)
        temp_back_colors = self.back_color_set.copy()  # avoid same back_color with C and avoid same color with font
        ...
        resume_back_color = Letters_Color_dict[A_letter].copy()
        if C_back_color in resume_back_color:
            resume_back_color.remove(C_back_color)
        A_back_color = random.choice(resume_back_color)
        A_font = random.choice(temp_font)
        A_img_name = A_G  + '_' + A_letter + '_' + A_back_color + '_' + A_size + '_' + A_font_color  + '_' + A_font + ".png"
        A_img_path = os.path.join(self.root, A_G, A_letter, A_back_color, A_size, A_font_color, A_font, A_img_name)

        # B has same size
        B_letter = random.choice(temp_Letters)
        while len(Letters_Color_dict[B_letter]) == 1 and Letters_Color_dict[B_letter][0] == C_back_color:
            B_letter = random.choice(temp_Letters)
        '''SAME size'''
        if len(Letters_Color_dict[B_letter])==1:
            B_G = 'G1'
        elif len(Letters_Color_dict[B_letter])==3:
            B_G = 'G2'
        else:
            B_G = 'G3'
        B_size = C_size
        B_font_color = random.choice(temp_font_color)
        resume_back_color = Letters_Color_dict[B_letter].copy()
        if C_back_color in resume_back_color:
            resume_back_color.remove(C_back_color)
        B_back_color = random.choice(resume_back_color)
        B_font = random.choice(temp_font)
        B_img_name = B_G + '_' + B_letter + '_' + B_back_color + '_' + B_size + '_' + B_font_color  + '_' + B_font + ".png"
        B_img_path = os.path.join(self.root, B_G, B_letter, B_back_color, B_size, B_font_color, B_font, B_img_name)

        # D has same font_color
        D_letter = random.choice(temp_Letters)
        while len(Letters_Color_dict[D_letter]) == 1 and Letters_Color_dict[D_letter][0] == C_back_color:
            D_letter = random.choice(temp_Letters)
        if len(Letters_Color_dict[D_letter])==1:
            D_G = 'G1'
        elif len(Letters_Color_dict[D_letter])==3:
            D_G = 'G2'
        else:
            D_G = 'G3'
        ...
        resume_back_color = Letters_Color_dict[D_letter].copy()
        if C_back_color in resume_back_color:
            resume_back_color.remove(C_back_color)
        D_back_color = random.choice(resume_back_color)
        D_font = random.choice(temp_font)
        D_img_name = D_G + '_' + D_letter + '_' + D_back_color + '_' + D_size + '_' + D_font_color  + '_' + D_font + ".png"
        D_img_path = os.path.join(self.root, D_G, D_letter, D_back_color, D_size, D_font_color, D_font, D_img_name)

        # E has same back_color
        E_letter = random.choice(temp_Letters)
        while C_back_color not in Letters_Color_dict[E_letter]:
            E_letter = random.choice(temp_Letters)
        if len(Letters_Color_dict[E_letter])==1:
            E_G = 'G1'
        elif len(Letters_Color_dict[E_letter])==3:
            E_G = 'G2'
        else:
            E_G = 'G3'
        E_size = random.choice(temp_Size)
        E_font_color = random.choice(temp_font_color)
        '''SAME back_color'''
        E_back_color = C_back_color
        E_font = random.choice(temp_font)
        E_img_name = E_G + '_' + E_letter + '_' + E_back_color + '_' + E_size + '_' + E_font_color + '_' + E_font + ".png"
        E_img_path = os.path.join(self.root, E_G, E_letter, E_back_color, E_size, E_font_color, E_font, E_img_name)

        # F has same font
        F_letter = random.choice(temp_Letters)
        while len(Letters_Color_dict[F_letter]) == 1 and Letters_Color_dict[F_letter][0] == C_back_color:
            F_letter = random.choice(temp_Letters)
        if len(Letters_Color_dict[F_letter])==1:
            F_G = 'G1'
        elif len(Letters_Color_dict[F_letter])==3:
            F_G = 'G2'
        elif len(Letters_Color_dict[F_letter])==6:
            F_G = 'G3'
        else:
            print(F_letter,Letters_Color_dict[F_letter])
        F_size = random.choice(temp_Size)
        F_font_color = random.choice(temp_font_color)
        resume_back_color = Letters_Color_dict[F_letter].copy()
        if C_back_color in resume_back_color:
            resume_back_color.remove(C_back_color)
        F_back_color = random.choice(resume_back_color)
        '''SAME font'''
        F_font = C_font
        F_img_name = F_G + '_' + F_letter + '_' + F_back_color + '_' + F_size + '_' + F_font_color + '_' + F_font + ".png"
        F_img_path = os.path.join(self.root, F_G,  F_letter, F_back_color, F_size, F_font_color, F_font, F_img_name)
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print(index,A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path, name_list)
            exit()

        labels = {'letter':[], 'size':[], 'bg':[], 'fg':[], 'style':[]}
        for name in name_list:
            labels['letter'].append(self.letter_dict[name.split('_')[1]])
            labels['size'].append(self.size_dict[name.split('_')[3]])
            labels['bg'].append(self.bg_dict1[name.split('_')[2]])
            labels['fg'].append(self.fg_dict[name.split('_')[4]])
            labels['style'].append(self.style_dict[name.split('_')[5].split('.')[0]])
...
def return_data(args):
    ...
    if name.lower() == '3dchairs':
        ...
        if args.use_server == True:
            if args.which_server == '15':
                root = '/lab/tmpig23b/u/zhix/font_dataset_D'
...
def val_data_loader(root = '', batch_size=1, workers=1, pin_memory=True):
    valdir = os.path.join('/lab/tmpig23b/u/zhix/classifiction_dataset/test')
...
def train_data_loader(root = '', batch_size=1, workers=1, pin_memory=True):
    valdir = os.path.join('/lab/tmpig23b/u/zhix/classifiction_dataset/bias')
    normalize = transforms.Normalize(mean=[0.5,0.5,0.5],
                                    std=[0.5,0.5,0.5])
