    ...
    def train(self):
        ...
        while not out:
            for sup_package in self.data_loader:
                ...
                # appe, pose, combine
                A_img = sup_package['A'][0]
                B_img = sup_package['B'][0]
                C_img = sup_package['C'][0]
                D_img = sup_package['D'][0]
                E_img = sup_package['E'][0]
                F_img = sup_package['F'][0]
                self.global_iter += 1
                pbar.update(1)
                ...
                synthesize Z
                '''
                A_img_name = sup_package['A'][1][0]
                B_img_name = sup_package['B'][1][0]
                C_img_name = sup_package['C'][1][0]
                D_img_name = sup_package['D'][1][0]
                E_img_name = sup_package['E'][1][0]
                F_img_name = sup_package['F'][1][0]

                A_z_save = str(A_z_vae.data.cpu().tolist()[0])
                B_z_save = str(B_z_vae.data.cpu().tolist()[0])
                C_z_save = str(C_z_vae.data.cpu().tolist()[0])
                D_z_save = str(D_z_vae.data.cpu().tolist()[0])
                E_z_save = str(E_z_vae.data.cpu().tolist()[0])
                F_z_save = str(F_z_vae.data.cpu().tolist()[0])
                f = open(self.log_dir + '/name_z.txt', 'a')
                # f.writelines(['\n', '{:s},{:.3f}\n{:s},{:.3f}\n{:s},{:.3f}\n{:s},{:.3f}\n{:s},{:.3f}\n{:s},{:.3f}'.format(A_img_name, A_z.data, B_img_name,B_z.data, C_img_name, C_z.data, D_img_name, D_z.data, E_img_name, E_z.data, F_img_name, F_z.data)])
                f.writelines([A_img_name + ',' + A_z_save[1:-1] + '\n', B_img_name + ',' + B_z_save[1:-1] + '\n',
                              C_img_name + ',' + C_z_save[1:-1] + '\n', D_img_name + ',' + D_z_save[1:-1] + '\n',
                              E_img_name + ',' + E_z_save[1:-1] + '\n', F_img_name + ',' + F_z_save[1:-1] + '\n'])
                f.close()
