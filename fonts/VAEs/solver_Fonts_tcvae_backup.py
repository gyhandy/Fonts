...
from dataset_cele2Fonts_backup import return_data
'''tcvae'''
...
class Solver(object):
    def __init__(self, args):
        ...
        self.btcvae_G = 1
        self.btcvae_B = 6
        kwargs_all = dict(rec_dist='gaussian', steps_anneal=10000)
        # kwargs_all = dict(rec_dist='bernoulli', steps_anneal=10000)
        self.loss_f = BtcvaeLoss(len(self.data_loader.dataset),
                          alpha=self.btcvae_A,
                          beta=self.btcvae_G,
                          gamma=self.btcvae_B,
                          **kwargs_all)
    ...
    def train(self):
        ...
        while not out:
            for x in self.data_loader:
                ...
                x_recon, mu, logvar, latent_sample = self.net(x)
                latent_dist = (mu, logvar)
                loss, rec_loss, tc_loss, mi_loss, dw_kl_loss = self.loss_f(x, x_recon, latent_dist, True,
                ...
                if self.viz_on and self.global_iter%self.gather_step == 0:
                    self.gather.insert(iter=self.global_iter,
                                       ...
                                       recon_loss=rec_loss.data, total_kld=loss.data,
                                       dim_wise_kld=tc_loss.data, mean_kld=mi_loss.data)
                    f = open(self.log_dir + '/log.txt', 'a')
                    f.writelines(
                        ['\n', '[{}] recon_loss:{}  total_kld:{}  mean_kld:{}'.format(
                            self.global_iter, rec_loss.data, loss.data, tc_loss.data)])
                    f.close()
