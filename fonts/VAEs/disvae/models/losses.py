...
'''
original one in paper
'''
class BtcvaeLoss_old(BaseLoss):
    """
...
class BtcvaeLoss(BaseLoss):
    ...
    def __call__(self, data, recon_batch, latent_dist, is_train, latent_sample=None):
        ...
        return loss, rec_loss, tc_loss, mi_loss, dw_kl_loss
...
def _reconstruction_loss(data, recon_data, distribution="bernoulli", storer=None):
    ...
    if distribution == "bernoulli":
        ...
        '''change another loss'''
        recon_data = F.sigmoid(recon_data)
        loss = F.mse_loss(recon_data, data, size_average=False)
