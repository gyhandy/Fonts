...
class dsprit_Nswap_imgfolder(Dataset):
    '''
    ...
    '''
    def __init__(self, root, transform=None):
        super(dsprit_Nswap_imgfolder, self).__init__()
        self.root = root
        self.transform = transform
        # self.paths = make_dataset(self.root)
        self.C_size = 737 # too much we fix it as the number of letters
        '''refer'''
        # Shape 3
    ...
    def findN(self, index):
        ...
        # A has same shape
        '''SAME shape  0: shape 1: scale 2:orientation 3:X 4: Y'''
        ...
        # return A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path
        return A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path, A_img_name, B_img_name, \
               C_img_name, D_img_name, E_img_name, F_img_name
    def __getitem__(self, index):
        '''there is a big while loop for choose category and training'''
        # A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path = self.findN(index)
        A_img_path, B_img_path, C_img_path, D_img_path, E_img_path, F_img_path, A_img_name, B_img_name, \
        C_img_name, D_img_name, E_img_name, F_img_name = self.findN(index)
        ...
        A = a = torch.from_numpy(np.array(A_img) / 255).unsqueeze(0).float()
        B = a = torch.from_numpy(np.array(B_img) / 255).unsqueeze(0).float()
        C = a = torch.from_numpy(np.array(C_img) / 255).unsqueeze(0).float()
        D = a = torch.from_numpy(np.array(D_img) / 255).unsqueeze(0).float()
        E = a = torch.from_numpy(np.array(E_img) / 255).unsqueeze(0).float()
        F = a = torch.from_numpy(np.array(F_img) / 255).unsqueeze(0).float()

        # return {'A': A, 'B': B, 'C': C, 'D': D, 'E': E, 'F': F}
        return {'A': (A, A_img_name), 'B': (B, B_img_name), 'C': (C, C_img_name), 'D': (D, D_img_name),
                'E': (E, E_img_name), 'F': (F, F_img_name)}
...
def return_data(args):
    ...
    if name.lower() == '3dchairs':
    ...
    elif name.lower() == 'dsprites_unsup_nswap':
        if args.use_server == True:
            if args.which_server == '15':
                root = '/home2/andy/dsprites_dataset'
                # root = '/home2/andy/fonts_dataset_half'
            elif args.which_server == '21':
                root = '/home2/andy/dsprites_dataset'
                # root = '/home2/andy/fonts_dataset_half'
        ...
        # dataset = ilab_sup_imgfolder(root, transform)
        # dataset = ilab_threeswap_imgfolder(root, transform)
        dataset = dsprit_Nswap_imgfolder(root, transform)
