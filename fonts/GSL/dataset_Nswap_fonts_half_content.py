...
class ilab_Nswap_imgfolder(Dataset):
    ...
    def __init__(self, root, transform=None):
        ...
        #                'silver': (192, 192, 192)}
        # now color 7
        self.Colors = {'red': (220, 20, 60), 'Yellow': (255, 255, 0), 'green': (0, 128, 0),
                       'cyan': (0, 255, 255),
                       'blue': (0, 0, 255), 'pink': (255, 192, 203), 'chocolate': (210, 105, 30)}
        self.Colors = list(self.Colors.keys())
        self.Colors_half = {'red': (220, 20, 60), 'Yellow': (255,255,0), 'green': (0,128,0), 'cyan' : (0,255,255)}
        self.Colors_half = list(self.Colors_half.keys())
        # size 3
        self.Sizes = {'small': 45, 'large': 60}
        self.Sizes = list(self.Sizes.keys())
        self.Sizes_half = {'large' : 60}
        self.Sizes_half = list(self.Sizes_half.keys())
        # style nearly over 100
        for roots, dirs, files in os.walk(os.path.join(self.root, 'A', 'small', 'red', 'Yellow')):
            cates = dirs
            break
        self.All_fonts = cates
        print(len(self.All_fonts))
        print(self.All_fonts, len(self.All_fonts))
        # for roots, dirs, files in os.walk(os.path.join('/home2/fonts_dataset_half', 'i', 'medium', 'red', 'orange')):
        #     cates = dirs
        #     break
        self.All_fonts_half = self.All_fonts[0:int(len(self.All_fonts)/2)]
        # letter 52
        # self.Letters = [chr(x) for x in list(range(65, 91)) + list(range(97, 123))]
        for roots, dirs, files in os.walk(self.root):
            contents = dirs
            break
        self.Letters = contents
        content_log = {}
        self.Letters_len_1 = []
        self.Letters_len_2 = []
        self.Letters_len_3 = []
        for letter in self.Letters:
            if len(letter)==1:
                self.Letters_len_1.append(letter)
            elif len(letter)==2:
                self.Letters_len_2.append(letter)
            elif len(letter) == 3:
                self.Letters_len_3.append(letter)
        assert len(self.Letters)==len(self.Letters_len_1)+len(self.Letters_len_2)+len(self.Letters_len_3)
        content_log['1'] = self.Letters_len_1
        content_log['2'] = self.Letters_len_2
        content_log['3'] = self.Letters_len_3
        content_log['All_fonts_half'] = self.All_fonts_half
        # for 2 Num content:
        self.Letters_group_1 = self.Letters_len_3 # half size
        content_log['half size'] = self.Letters_group_1
        self.Letters_group_2 = self.Letters_len_3 # half font color
        content_log['half font color'] = self.Letters_group_2
        # for 3 Num content
        self.Letters_group_3 = self.Letters_len_3 # half back color
        content_log['half back color'] = self.Letters_group_3
        self.Letters_group_4 = self.Letters_len_3  # half style
        content_log['half style'] = self.Letters_group_4
        self.Letters_group_5 = self.Letters_len_3 + self.Letters_len_2 # all half
        content_log['all half'] = self.Letters_group_5

        # 保存
        dict_name = content_log
        f = open('content_log.txt', 'w')
    ...
    def findN(self, index):
        ...
        SIZE = False # False means the selected attribute in half_set
        FONTCOLOR = False
        BACKCOLOR = False
        FONT = False
        C_letter  = self.Letters[index]
        if C_letter in self.Letters_group_1 or C_letter in self.Letters_group_5:
            SIZE = True
            C_size = random.choice(self.Sizes_half)
        else:
            C_size = random.choice(self.Sizes)
        if C_letter in self.Letters_group_2 or C_letter in self.Letters_group_5:
            FONTCOLOR = True
            C_font_color = random.choice(self.Colors_half)
        else:
            C_font_color = random.choice(self.Colors)
        if C_letter in self.Letters_group_3 or C_letter in self.Letters_group_5:
            BACKCOLOR = True
            resume_colors = self.Colors_half.copy()
        else:
            resume_colors = self.Colors.copy()
        if C_font_color in resume_colors:
            resume_colors.remove(C_font_color)
        C_back_color = random.choice(resume_colors)
        if C_letter in self.Letters_group_4 or C_letter in self.Letters_group_5:
            FONT = True
            C_font = random.choice(self.All_fonts_half)
        else:
        ...
        temp_Size_half = self.Sizes_half

        temp_font_color = self.Colors.copy()# avoid same font_color with C
        temp_font_color.remove(C_font_color)
        temp_font_color_half = self.Colors_half.copy()
        if C_font_color in temp_font_color_half:
            temp_font_color_half.remove(C_font_color)

        temp_back_color = self.Colors.copy()  # avoid same back_color with C and avoid same color with font
        temp_back_color.remove(C_back_color)
        temp_back_color_half = self.Colors_half.copy()
        if C_back_color in temp_back_color_half:
            temp_back_color_half.remove(C_back_color)

        temp_font = self.All_fonts.copy()  # avoid same font with C
        temp_font.remove(C_font)
        temp_font_half = self.All_fonts_half.copy()
        if C_font in temp_font_half:
            temp_font_half.remove(C_font)

        # A has same content
        '''SAME content'''
        A_letter = C_letter
        if A_letter in self.Letters_group_1 or A_letter in self.Letters_group_5:
            A_size = random.choice(temp_Size_half)
        else:
            A_size = random.choice(temp_Size)
        if A_letter in self.Letters_group_2 or A_letter in self.Letters_group_5:
            A_font_color = random.choice(temp_font_color_half)
        else:
            A_font_color = random.choice(temp_font_color)
        if A_letter in self.Letters_group_3 or A_letter in self.Letters_group_5:
            resume_colors = temp_back_color_half.copy()
        else:
            resume_colors = temp_back_color.copy()
        if A_font_color in resume_colors:
            resume_colors.remove(A_font_color)
        A_back_color = random.choice(resume_colors)
        if A_letter in self.Letters_group_4 or A_letter in self.Letters_group_5:
            A_font = random.choice(temp_font_half)
        else:
        ...
        # B has same size
        if SIZE: # selected size in half_set
            B_letter = random.choice(temp_Letters)
        else: # selected size no in half_set
            temp = list(self.Letters_group_2 + self.Letters_group_3 + self.Letters_group_4)
            if C_letter in temp:
                temp.remove(C_letter)
            B_letter = random.choice(temp)
        '''SAME size'''
        B_size = C_size
        if B_letter in self.Letters_group_2 or B_letter in self.Letters_group_5:
            B_font_color = random.choice(temp_font_color_half)
        else:
            B_font_color = random.choice(temp_font_color)
        if B_letter in self.Letters_group_3 or B_letter in self.Letters_group_5:
            resume_colors = temp_back_color_half.copy()
        else:
            resume_colors = temp_back_color.copy()
        ...
        if B_letter in self.Letters_group_4 or B_letter in self.Letters_group_5:
            B_font = random.choice(temp_font_half)
        else:
        ...
        if FONTCOLOR:
            ...
            temp = list(self.Letters_group_1 + self.Letters_group_3 + self.Letters_group_4)
            if C_letter in temp:
                temp.remove(C_letter)
            D_letter = random.choice(temp)
        if D_letter in self.Letters_group_1 or D_letter in self.Letters_group_5:
            D_size = random.choice(temp_Size_half)
        else:
            D_size = random.choice(temp_Size)
        '''SAME font_color'''
        D_font_color = C_font_color
        if D_letter in self.Letters_group_3 or D_letter in self.Letters_group_5:
            resume_colors = temp_back_color_half.copy()
        else:
            resume_colors = temp_back_color.copy()
        ...
        if D_letter in self.Letters_group_4 or D_letter in self.Letters_group_5:
            D_font = random.choice(temp_font_half)
        else:
        ...
        if BACKCOLOR:
            ...
            temp = list(self.Letters_group_1 + self.Letters_group_2 + self.Letters_group_4)
            if C_letter in temp:
                temp.remove(C_letter)
            E_letter = random.choice(temp)
        if E_letter in self.Letters_group_1 or E_letter in self.Letters_group_5:
            E_size = random.choice(temp_Size_half)
        else:
            E_size = random.choice(temp_Size)
        if E_letter in self.Letters_group_2 or E_letter in self.Letters_group_5:
            resume_colors = temp_font_color_half.copy()
            if C_back_color in resume_colors:
        ...
        if E_letter in self.Letters_group_4 or E_letter in self.Letters_group_5:
            E_font = random.choice(temp_font_half)
        else:
        ...
        if FONT:
            ...
            temp = list(self.Letters_group_1 + self.Letters_group_2 + self.Letters_group_3)
            if C_letter in temp:
                temp.remove(C_letter)
            F_letter = random.choice(temp)
        if F_letter in self.Letters_group_1 or F_letter in self.Letters_group_5:
            F_size = random.choice(temp_Size_half)
        else:
            F_size = random.choice(temp_Size)
        if F_letter in self.Letters_group_2 or F_letter in self.Letters_group_5:
            F_font_color = random.choice(temp_font_color_half)
        else:
            F_font_color = random.choice(temp_font_color)
        if F_letter in self.Letters_group_3 or F_letter in self.Letters_group_5:
            resume_colors = temp_back_color_half.copy()
        else:
            resume_colors = temp_back_color.copy()
...
def return_data(args):
    ...
    elif name.lower() == 'fonts_3content':
        if args.use_server == True:
            if args.which_server == '15':
            ...
            # root = '/home2/fonts_3content'
            root = '/home2/fonts_dataset_version0upper'
