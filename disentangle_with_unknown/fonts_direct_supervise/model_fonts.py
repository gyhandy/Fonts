...
class Classifier_latent(nn.Module):
    def __init__(self, input_size=20, output_size=52):
...
class Classifier_image(nn.Module):
    def __init__(self, num_classes=52, init_weights=False):
        super(Classifier_image, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(3, 48, kernel_size=11, stride=4, padding=2),  # input[3, 128, 128], output[48, 31, 31]
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),                  # output[48, 15, 15]
            nn.Conv2d(48, 128, kernel_size=5, padding=2),           # output[128, 15, 15]
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=3, stride=2),                  # output[128, 7, 7]
            nn.Conv2d(128, 192, kernel_size=3, padding=1),          # output[192, 7, 7]
            nn.ReLU(inplace=True),
            nn.Conv2d(192, 192, kernel_size=3, padding=1),          # output[192, 7, 7]
            nn.ReLU(inplace=True),
            nn.Conv2d(192, 128, kernel_size=3, padding=1),          # output[128, 7, 7]
            nn.ReLU(inplace=True),
