...
class Solver(object):
    def __init__(self, args):
        ...
        self.lambda_dif = args.lambda_dif
    ...
    def train(self):
        ...
        while not out:
            for sup_package in self.data_loader:
                ...
                # swap_different_loss
                # C A different style 5
                dif_A5Co_combine = torch.cat((C_z_1, C_z_2, C_z_3, C_z_4, A_z_5), dim=1)
                dif_mid_A5Co = self.Autoencoder.fc_decoder(dif_A5Co_combine)
                dif_mid_A5Co = dif_mid_A5Co.view(dif_A5Co_combine.shape[0], 256, 8, 8)
                dif_A5Co = self.Autoencoder.decoder(dif_mid_A5Co)

                dif_AoC5_combine = torch.cat((A_z_1, A_z_2, A_z_3, A_z_4, C_z_5), dim=1)
                dif_mid_AoC5 = self.Autoencoder.fc_decoder(dif_AoC5_combine)
                dif_mid_AoC5 = dif_mid_AoC5.view(dif_AoC5_combine.shape[0], 256, 8, 8)
                dif_AoC5 = self.Autoencoder.decoder(dif_mid_AoC5)
                ...
                # 4. different_combine_loss

                dif_A5Co_loss = torch.mean(torch.abs(C_img - dif_A5Co))
                dif_AoC5_loss = torch.mean(torch.abs(A_img - dif_AoC5))
                combine_dif_loss = dif_A5Co_loss + dif_AoC5_loss

                # whole loss
                vae_unsup_loss = recon_loss + self.lambda_combine * combine_sup_loss - self.lambda_dif * combine_dif_loss
                ...
                f.writelines(['\n',
                              '[{}] recon_loss:{:.3f}  combine_sup_loss:{:.3f}  combine_dif_loss:{:.3f}'.format(
                                  self.global_iter, recon_loss.data, combine_sup_loss.data, combine_dif_loss.data)])
                f.close()

                if self.viz_on and self.global_iter % self.gather_step == 0:
                    self.gather.insert(iter=self.global_iter, recon_loss=recon_loss.data,
                                       combine_sup_loss=combine_sup_loss.data,
                                       combine_dif_loss=combine_dif_loss.data)
                ...
                if self.global_iter % self.display_step == 0:
                    ...
                    if self.viz_on:
                        ...
                        combine show
                        '''
                        self.gather.insert(combine_supimages=F.sigmoid(F5Co_2C).data)
                        self.gather.insert(combine_supimages=F.sigmoid(FoC5_2F).data)
                        self.viz_combine_recon()
                        '''
                        swap diff show
                        '''
                        self.gather.insert(combine_difimages=F.sigmoid(dif_A5Co).data)
                        self.gather.insert(combine_difimages=F.sigmoid(dif_AoC5).data)
                        self.viz_diff()
                        # self.viz_combine(x)
                        self.gather.flush()
    ...
    def save_sample_img(self, tensor, mode):
        ...
        if mode == 'recon':
            ...
            image_F5Co_2C = image[0].squeeze(0)  # remove the fake batch dimension
            image_FoC5_2F = image[1].squeeze(0)  # remove the fake batch dimension

            image_F5Co_2C = unloader(image_F5Co_2C)
            image_FoC5_2F = unloader(image_FoC5_2F)

            image_F5Co_2C.save(os.path.join(dir, '{}-F5Co_2C.png'.format(self.global_iter)))
            ...
            image_dif_A5Co = image[0].squeeze(0)  # remove the fake batch dimension
            image_dif_AoC5 = image[1].squeeze(0)  # remove the fake batch dimension

            image_dif_A5Co = unloader(image_dif_A5Co)
            image_dif_AoC5 = unloader(image_dif_AoC5)

            image_dif_A5Co.save(os.path.join(dir, '{}-dif_A5Co.png'.format(self.global_iter)))
            image_dif_AoC5.save(os.path.join(dir, '{}-dif_AoC5.png'.format(self.global_iter)))
    ...
    def viz_combine_recon(self):
        # self.net_mode(train=False)
        F5Co_2C = self.gather.data['combine_supimages'][0][:100]
        F5Co_2C = make_grid(F5Co_2C, normalize=True)
        FoC5_2F = self.gather.data['combine_supimages'][1][:100]
        FoC5_2F = make_grid(FoC5_2F, normalize=True)
        images = torch.stack([F5Co_2C, FoC5_2F], dim=0).cpu()
    ...
    def viz_diff(self):
        # self.net_mode(train=False)
        dif_A5Co = self.gather.data['combine_difimages'][0][:100]
        dif_A5Co = make_grid(dif_A5Co, normalize=True)
        dif_AoC5 = self.gather.data['combine_difimages'][1][:100]
        dif_AoC5 = make_grid(dif_AoC5, normalize=True)
        images = torch.stack([dif_A5Co, dif_AoC5], dim=0).cpu()
    ...
    def viz_lines(self):
        ...
        combine_dif_loss = torch.stack(self.gather.data['combine_dif_loss']).cpu()
        ...
        if self.win_combine_dif is None:
            self.win_combine_dif = self.viz.line(
                X=iters,
                Y=combine_dif_loss,
                env=self.viz_name + '_lines',
                opts=dict(
                    ...
                    legend=legend[:self.z_dim],
                    xlabel='iteration',
                    title='combine_dif_loss', ))
        else:
            self.win_combine_unsup = self.viz.line(
                X=iters,
                Y=combine_dif_loss,
                env=self.viz_name + '_lines',
                win=self.win_combine_dif,
                update='append',
                opts=dict(
                    width=400,
                    height=400,
